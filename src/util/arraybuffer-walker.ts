...
export class ArrayBufferWalker {
  ...
  writeCRC() {
    ...
    let crc = crc32(
      this.array,
      this.crcStartOffset,
      this.offset - this.crcStartOffset
    );
  ...
  pauseAdler() {
    ...
    this.savedAdlerValue = adler32_buf(
      this.array,
      this.adlerStartOffset,
      this.offset - this.adlerStartOffset,
      this.savedAdlerValue
    );
  ...
  writeAdler(walker: ArrayBufferWalker) {
    ...
    if (this.adlerStartOffset === undefined) {
      walker.writeUint32(this.savedAdlerValue as number);
      this.savedAdlerValue = undefined;
      return;
    }

    let adler = adler32_buf(
      this.array,
      this.adlerStartOffset,
      this.offset - this.adlerStartOffset,
      this.savedAdlerValue
